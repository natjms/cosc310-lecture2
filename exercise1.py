...
def available_under(menu: list[dict], limit: float) -> list[dict]:
    ...
    # TO/DO: filter items priced under $10, then sort by price.
    filtered_items = [item for item in menu if item['price'] < limit]
...
def main() -> None:
    ...
    for item in available_under(menu, 10.00):
        # TO/DO: print name and price using an f-string.
